...
export function buildStockSearchTargets(
  ...
  for (const platform of platforms) {
    ...
    if (!added) warnings.push(`${platform} does not support ${kind} media, skipped`);
  }
...
function addUnavailableWarning(warnings: string[], target: SearchTarget): void {
  const message = target.platform === 'freesound'
    ? 'Freesound is not configured, so audio and music cannot be searched'
    : `${target.platform} is not configured, ${target.kind} search skipped`;
...
export async function searchStockMedia(
  ...
  if (request.orientation && !orientation) warnings.push(`Unsupported orientation "${request.orientation}", the orientation filter was ignored`);
  ...
  for (const target of planned.targets) {
    if (target.platform === 'pexels' && options.pexelsApiKey) {
      ...
      if (orientation === 'square') {
        const warning = 'The official Pixabay API does not support a square orientation filter; the other filters were kept';
