      ...
      server.middlewares.use('/generate/music', async (req, res) => {
        ...
        try {
          ...
          if (input.provider === 'minimax' && !options.minimaxApiKey) {
            throw new Error('MiniMax is not configured. Set MINIMAX_API_KEY in .env.local or the settings panel.');
          }
          if (input.provider === 'mureka' && !options.apiKey) {
            throw new Error('Mureka is not configured. Set MUREKA_API_KEY in .env.local or the settings panel.');
          }
          if (input.provider === 'atlas' && !options.atlasApiKey) {
            throw new Error('Atlas Cloud is not configured. Set ATLASCLOUD_API_KEY in .env.local or the settings panel.');
          }
          if (input.provider === 'sonilo' && !options.soniloApiKey) {
            throw new Error('Sonilo is not configured. Set SONILO_API_KEY in .env.local or the settings panel.');
          }
