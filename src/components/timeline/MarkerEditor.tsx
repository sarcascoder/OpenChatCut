  ...
  return (
    <div style={{ position: 'absolute', top: 40, left: 12, zIndex: 20, width: 260, background: theme.panelAlt, border: `0.5px solid ${theme.border}`, borderRadius: 5, padding: 12, boxShadow: `0 8px 28px ${themeAlpha.shadow(0.45)}` }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 8, fontSize: 12, color: theme.textDim }}>
        ...
        Marker · {fmt(editing.fromFrame, fps)}
      </div>
      <textarea autoFocus value={editing.note} onChange={(e) => commands.updateMarker(editing.id, { note: e.target.value })} rows={3} placeholder="Note…"
      ...
      <div style={{ display: 'flex', gap: 6, alignItems: 'center', marginBottom: 9, fontSize: 12, color: theme.textDim }}>
        ...
        <span>sec{editing.durationFrames > 0 ? ' (range)' : ' (point)'}</span>
      </div>
      <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
        <button onClick={() => { commands.removeMarker(editing.id); onClose(); }} style={{ ...toolBtn, color: theme.accent, fontSize: 12 }}>Delete</button>
        <span style={{ flex: 1 }} />
        <button onClick={onClose} style={{ background: theme.accent, border: 'none', borderRadius: 6, color: theme.onAccent, cursor: 'pointer', fontSize: 12, padding: '4px 12px' }}>Done</button>
      </div>
    </div>
  );
}
