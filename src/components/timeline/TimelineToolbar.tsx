  ...
  return (
    <>
      ...
      <div className="cc-timeline-tool-group">
        ...
        <ToolSep />
        <TB icon="cursor" title="Selection mode (V): drag to move / trim edges" active={editMode === 'selection'} onClick={() => invokeAction('interaction-mode-selection', undefined, 'toolbar')} />
        <TB icon="trim" title="Trim mode (N): trim clip edges; later clips follow to close the gap (ripple)" active={editMode === 'trim'} onClick={() => invokeAction('interaction-mode-trim', undefined, 'toolbar')} />
        <TB
          icon="swap"
          title="Slip mode (U): keep timeline position and duration, drag only the source range"
        ...
        <TB
          ...
          title={editMode === 'rate-stretch'
            ? 'Exit Rate Stretch and return to Selection mode'
            : 'Rate Stretch: drag either clip edge to preserve the source range and change playback speed'}
        ...
        />
        <TB icon="blade" title="Blade mode (B): click a clip to split it there" active={editMode === 'blade'} onClick={() => invokeAction('interaction-mode-blade', undefined, 'toolbar')} />
        <TB icon="pencil" title="Pen mode (P): click the selected clip to draw keyframes (visual clips = opacity, audio clips = volume; vertical = value; drag a point to change frame/value, right-click to delete)" active={editMode === 'pen'} onClick={() => invokeAction('interaction-mode-pen', undefined, 'toolbar')} />
        <TB icon="scissors" title="Split the selected clip at the playhead (C)" onClick={() => invokeAction('split', undefined, 'toolbar')} />
        <TB icon="magnet" title={snapping ? 'Snapping: on (S)' : 'Snapping: off (S)'} active={snapping} onClick={() => invokeAction('snapping', undefined, 'toolbar')} />
        <ToolSep />
        <TimelineMoreControl
          placeMode={placeMode}
        ...
        <span className="cc-mic-group">
          <TB icon="mic" active={recorder.recording}
            title={recorder.recording ? '● Recording, click to stop' : recorder.error ? `Recording failed: ${recorder.error}` : 'Record voiceover (microphone → audio track)'}
            disabled={!canRecord} onClick={recorder.toggle} />
          <Icon name="chevronDown" size={13} />
        </span>
        {recorder.recording && <span title="Recording" style={{ width: 8, height: 8, borderRadius: '50%', background: theme.accent, animation: 'cc-rec-pulse 1.2s ease-out infinite', flexShrink: 0 }} />}
      ...
      <TB icon="zoomOut" title="Zoom out timeline (⌘−)" tipRight onClick={() => invokeAction('zoom-out', undefined, 'toolbar')} />
      <input type="range" min={MIN_TIME_ZOOM} max={6} step={0.01} value={zoom} onChange={(e) => setZoom(Number(e.target.value))}
        title="Zoom timeline" className="cc-timeline-zoom" />
      <TB icon="zoomIn" title="Zoom in timeline (⌘+)" tipRight onClick={() => invokeAction('zoom-in', undefined, 'toolbar')} />
      <TB icon="fit" title="Fit to view (⇧Z)" tipRight onClick={() => invokeAction('zoom-fit', undefined, 'toolbar')} />
      <label className="cc-aspect-select cc-tip cc-tip-r" data-tip="Aspect ratio">
        <Icon name="aspect" size={16} />
        <select aria-label="Aspect ratio" value={ASPECT_PRESETS.find((preset) => preset.width === state.width && preset.height === state.height)?.label ?? ''}
          onChange={(event) => {
          ...
          <optgroup label="Aspect ratio">{ASPECT_PRESETS.map((preset) => <option key={preset.label} value={preset.label}>{preset.label}</option>)}</optgroup>
          <optgroup label="Content fit"><option value="__contain__">Letterbox</option><option value="__cover__">Crop</option></optgroup>
        </select>
      </label>
      <button className={`cc-caption-toggle cc-tip cc-tip-r${captionsVisible ? ' active' : ''}`} data-tip={hasCaptions
        ? captionTracks.length
          ? 'Captions'
          : `Caption display (${textClipCount} text assets on screen, no caption track data; turning off hides them too)`
        : 'Caption display (no captions or text assets yet; transcribe first or let the Agent generate)'} aria-label="Captions" disabled={!hasCaptions} onClick={() => commands.setCaptionsHidden(captionsVisible)}><Icon name="captions" size={17} /><span>{captionsVisible ? 'On' : 'Off'}</span><Icon name="chevronDown" size={13} /></button>
      <TB icon="fullscreen" title="Fullscreen preview" tipRight onClick={() => invokeAction('fullscreen', undefined, 'toolbar')} />
      </div>
      </div>
