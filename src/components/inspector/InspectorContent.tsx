...
function InspectorTabBar({ id, activeTab, available, onChange }: {
  ...
  return (
    <div className="cc-insp-tabs" role="tablist" aria-label="Property categories">
...
function BasicTab({ panel, item, schema, playheadLocal }: InspectorContentProps) {
  ...
  return (
    <>
      ...
      {panel.selectedItems.every((entry) => entry.kind !== 'audio') && <><SectionLabel onReset={() => panel.onResetItemKeyframes(transformProps)} resetDisabled={resetDisabled && !transformProps.some((prop) => isMixed(panel, (entry) => getKeyframePropertyDefinition(prop).getBaseValue(entry)))}>Transform</SectionLabel><TransformControl item={item} mixed={(prop) => {
        const definition = getKeyframePropertyDefinition(prop);
...
function AnimationTab({ panel, item }: InspectorContentProps) {
  ...
  return (
    <>
      ...
      <SectionLabel onReset={() => panel.onItemFadeChange({ fadeInFrames: 0, fadeOutFrames: 0 })} resetDisabled={(item.fadeInFrames ?? 0) === 0 && (item.fadeOutFrames ?? 0) === 0 && !isMixed(panel, (entry) => [entry.fadeInFrames ?? 0, entry.fadeOutFrames ?? 0])}>Fade</SectionLabel>
...
function InspectorHint({ item, count }: { item: TimelineItem; count: number }) {
  ...
  return (
    <div className="cc-insp-scope">
      {count > 1 && <strong>{`${count} clips selected`}</strong>}
      <span className="cc-insp-scope-kind">{labels[item.kind] ?? 'Clip'}</span>
      <span>{sourceBacked
        ? 'Affects only this timeline clip; the source file in the media pool is unchanged.'
        : 'Affects only this timeline clip.'}</span>
    </div>
  );
}
