...
export function CaptionInspectorControls({ selection, onUpdate }: CaptionInspectorControlsProps) {
  ...
  return (
    <div className="cc-insp-groups" data-caption-inspector="true">
      ...
      <RangeRow label="Weight" value={target.preset.fontWeight} min={100} max={900} step={100}
        display={String(target.preset.fontWeight)} onChange={(value) => patchStyle({ fontWeight: value })} />
      <RangeRow label="Text outline" value={target.preset.strokeWidth} min={0} max={16} step={0.5}
        display={`${Math.round(target.preset.strokeWidth * 10) / 10}px`} onChange={(value) => patchStyle({ strokeWidth: value })} />
      <RangeRow label="Outline opacity" value={strokeOpacity} min={0} max={1} step={0.01}
      ...
      <RangeRow label="Border stroke" value={boxBorderWidth} min={0} max={16} step={0.5}
      ...
      <RangeRow label="Border radius" value={boxBorderRadius} min={0} max={48} step={1}
        display={`${Math.round(boxBorderRadius)}px`} onChange={(value) => patchStyle({ boxBorderRadius: value })} />
      <RangeRow label="Border shadow" value={boxShadowSize} min={0} max={48} step={1}
      ...
      <div className="cc-insp-color-row cc-caption-color-row">
        ...
        <CaptionColorInput label="Text outline" value={target.preset.strokeColor} fallback="#000000"
          onChange={(value) => patchStyle({ strokeColor: value })} />
        <CaptionColorInput label="Text shadow" value={shadowColor(target.preset.textShadow)} fallback="#000000"
          onChange={(value) => patchStyle({ textShadow: withShadowColor(target.preset.textShadow, value) })} />
      </div>
      <div className="cc-insp-color-row cc-caption-color-row">
        <CaptionColorInput label="Border color" value={backgroundColor} fallback="#000000"
          onChange={(value) => patchStyle(target.preset.wholeLine ? { background: value } : { highlightBackground: value })} />
        <CaptionColorInput label="Border stroke" value={boxBorderColor} fallback="#000000"
          onChange={(value) => patchStyle({ boxBorderColor: value })} />
        <CaptionColorInput label="Border shadow" value={shadowColor(boxShadow || target.preset.textShadow)} fallback="#000000"
      ...
      <RangeRow label="Zoom" value={scale} min={0.25} max={4} step={0.01}
        display={`${Math.round(scale * 100)}%`} onChange={(value) => patchLayout({ scale: value })} />
      <RangeRow label="Horizontal" value={x} min={-1} max={1} step={0.01}
        display={`${Math.round(x * 100)}%`} onChange={(value) => patchLayout({ offsetXRatio: value })} />
      <RangeRow label="Vertical" value={y} min={-1} max={1} step={0.01}
