...
assert.equal(matches[0]?.sampleTime, 0, 'the frame kept is the closest-matching one in that asset');
...
assert.deepEqual(weak.map((item) => item.assetId), ['city'], 'entries clearly worse than the best hit are not padded in');
...
assert.equal(orthogonal.length, 3, 'one frame kept per asset, three in total');
