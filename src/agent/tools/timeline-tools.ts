...
function resolveDims(a: { ratio?: unknown; width?: unknown; height?: unknown }): { width: number; height: number } | null | { error: string } {
  if (typeof a.ratio === 'string' && a.ratio) {
    ...
    return preset ? { width: preset.width, height: preset.height } : { error: `unknown ratio ${a.ratio} (choose from ${ASPECT_PRESETS.map((p) => p.label).join('/')})` };
  }
...
export async function execTimelineTool(name: string, args: Args, ctx: AgentContext): Promise<unknown> {
  ...
  switch (String(args.action)) {
    ...
    case 'delete': {
      ...
      return { ok: deleted.length > 0, deleted, ...(kept.length ? { kept, note: 'skipped: at least one sequence must remain, or it is referenced by a nested instance, or it was not found' } : {}), ...(blocked.length ? { blocked } : {}) };
    }

    default:
      return { error: `unknown action ${args.action} (choose from list/create/duplicate/switch/update/delete/insert)` };
  }
}
