...
function resolveTranscriptSegments(state: TimelineState, spec: string, trackFilter?: string): SegmentAnchor | { error: string } {
  ...
  if (!sns) return { error: `transcriptSegments "${spec}" could not be parsed - use the [sN] numbers printed by read_script, e.g. "3", "3-5" or "2,4-6"` };
  const model = buildModel(state);
  const tracks = trackFilter ? model.filter((t) => t.track.toLowerCase() === trackFilter.toLowerCase()) : model;
  if (trackFilter && !tracks.length) return { error: `transcriptTrack "${trackFilter}" does not exist or that track has no content` };
  ...
  if (!candidates.length) {
    return { error: `no transcript region contains all of segments ${sns.join(',')} - run read_script first to check the [sN] numbers${trackFilter ? '' : ', or pass transcriptTrack to narrow the range'}` };
  }
  if (candidates.length > 1) {
    return { error: `segments ${sns.join(',')} appear in several transcript regions (${candidates.map((c) => `${c.track}:${c.itemId.slice(0, 8)}`).join(' / ')}) - pass transcriptTrack to disambiguate, or give fromFrame directly` };
  }

  const cand = candidates[0]!;
  const item = state.items.find((it) => it.id === cand.itemId);
  if (!hasOperationalTranscript(item)) return { error: `clip ${cand.itemId} behind that transcript region no longer has a current transcript, run read_script again` };
  ...
  if (!f0 || !f1) return { error: `the words of segments ${sns.join(',')} were deleted or fall outside the playback range, so no frame can be located - check with read_script and retry` };
