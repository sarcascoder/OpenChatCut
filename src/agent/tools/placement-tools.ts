...
export async function execPlaceGraphicsTool(name: string, args: Args, ctx: AgentContext): Promise<unknown> {
  ...
  if (requested && !targets.length) {
    return { error: `No graphic clip found for ${requested} (supported kinds: motion-graphic/text/solid)`, available: graphics.map((g) => ({ itemId: g.id, name: g.name, kind: g.kind })) };
  }
  if (!targets.length) {
    return { ok: true, adjusted: 0, note: 'The timeline has no overlay graphics to place (motion-graphic/text/solid).' };
  }
  ...
  for (const item of targets) {
    ...
    if (geometry === undefined) {
      ...
      if (!asset) {
        skipped.push(`${item.name} (the video is not in the media pool)`);
        continue;
      }
    ...
    if (!transform) {
      skipped.push(`${item.name} (the safe area is too small to fit it)`);
      continue;
    }
  ...
  return {
    ...
    note: placed.length
      ? `Moved ${placed.length} graphics into the safe area (clear of faces/subjects).`
      : 'No graphics were moved; ' + (skipped.length ? `skipped: ${skipped.join('; ')}` : 'the safe areas were already usable.'),
  };
}
