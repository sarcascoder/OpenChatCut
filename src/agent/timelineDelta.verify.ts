...
// ── Read-only: status unchanged → null (do not add any fields to the result) ──
{
  ...
  assert.equal(describeTimelineDelta(snapshotTimeline(s), s), null, 'no change produces no delta');
}
...
// ── Create new / change track / change length → enter clips one by one ──
{
  ...
  assert.equal(d.clips?.length, 3, 'all three kinds of change land in clips');
  assert.deepEqual(d.clips?.map((c) => c.id).sort(), ['a', 'b', 'c']);
  assert.equal(d.shifted, undefined, 'none of these is a pure shift');
  const a = d.clips!.find((c) => c.id === 'a')!;
  assert.equal(a.durationInFrames, 45, 'clips carry the new state, not the old one');
  assert.equal(a.track, 'V1', 'track reports the alias (bottom video track = V1), matching read_project');
}
...
// ── Corrugation: Same track and same displacement ≥3 → compressed into one rule, not listed one by one ──
{
  ...
  const d = describeTimelineDelta(before, after)!;
  assert.equal(d.clips, undefined, 'a pure shift does not land in clips');
  assert.deepEqual(d.shifted, [{ track: 'V1', fromFrame: 30, by: -30, count: 4 }], 'collapsed into a single rule');
  assert.deepEqual(d.removedItemIds, ['a'], 'removals are reported separately');
}
...
// ── Sporadic displacement (<3) → Return to enumeration one by one, do not press the rules ──
{
  ...
  const d = describeTimelineDelta(before, after)!;
  assert.equal(d.shifted, undefined, '2 items are not collapsed into a rule');
...
// ── Different rails/different displacements are grouped into groups ──
{
  ...
  const d = describeTimelineDelta(before, after)!;
  assert.equal(d.shifted?.length, 2, 'two tracks, two rules');
...
// ── Upper limit: if there are more than 30 entries, only the top 30 will be listed and prompted to re-read──
{
  ...
  const d = describeTimelineDelta(before, after)!;
  assert.equal(d.clips?.length, 30, 'at most 30 are listed');
  assert.match(d.notes?.join(' ') ?? '', /40 clips changed in total/, 'the note gives the total and asks for a re-read');
}
...
// ── New track reporting + track composition change reminder ──
{
  ...
  const d = describeTimelineDelta(before, after)!;
  assert.deepEqual(d.createdTracks, ['V2'], 'the new track is reported (by alias: the new track sits on top = V2)');
  assert.match(d.notes?.join(' ') ?? '', /track layout changed/, 'reminds the reader to re-confirm track positions');
}
...
// ── Ripple removal by real reducer: difference is consistent with actual result ──
{
  ...
  assert.deepEqual(d.removedItemIds, ['a'], 'the real reducer removal');
  assert.equal(d.shifted?.length, 1, 'the ripple collapses into a single rule');
  assert.equal(d.shifted![0]!.count, 3, 'the 3 following clips shift');
  assert.equal(d.shifted![0]!.by, -30, 'each moves 30 frames left');
  // Rules can infer the true position
  for (const it of s1.items) {
    const was = before.placements.get(it.id)!;
    assert.equal(it.startFrame, was.startFrame + d.shifted![0]!.by, `${it.id} position is derivable from the rule`);
  }
}
