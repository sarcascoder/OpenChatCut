...
name: shader-gen
description: |
  AI shader generator for WebGL video effects, transitions, masks, and color grading (LUT / color grade / cinematic look / film look). Use when the user wants a video effect (filter / VFX), a transition (crossfade / wipe / cube / 3d), a mask (matte / reveal), a zoom / push-in (dolly-in / punch-in), or a color grade — try the built-in effects (zoom, builtin LUTs) before generating a new shader.
user-invocable: true
---
...
Use the user's language. For example: "Generating new effects/transitions is still in beta and can be unstable. If you still want to go ahead, I can build it for you." Skip if user already acknowledged in the same request.
