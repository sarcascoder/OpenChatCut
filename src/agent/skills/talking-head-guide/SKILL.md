...
- **A-roll editing** (covers **filler removal, pauses, and repeats**) — transcript-based speech editing. Common operations include cleanup, highlight extraction, restructure, opening hook, and others as needed for the aligned outcome.
- **Motion graphics overlay** (when showing this to the user, write the full name **Motion Graphics**; do not shorten it to "MG", and do not call it "motion effects", "caption bar", "animated captions", or anything else) — reinforce key information, structured content, and topic transitions with on-screen motion graphics
...
> In widget options / choices options / conversational copy, **use exactly the product terms above** — don't reword them yourself, or they won't line up with the rest of the product.
...
- `and then let's look at the second point` -> keep `and then`; it marks sequence.
...
2. For cleanup tasks, run the mechanical cleanup pass before semantic editing when fixed fillers or long pauses are present. Use `clean_script` for fixed hesitation sounds (`um`, `uh`, `er`, `ah`, `erm`, `hmm`) and batch pause compression. If both are present, use the default `clean_script` pass so both are handled together. Do not use this step for context-dependent fillers, retakes, repeated sentences, or anything that needs meaning.
...
- Single-word audible deletion is supported with inline strike syntax, such as `[s1] over the ~~um~~ past month`.
...
When the user has **two or more cameras recording the same moment** — cues like "both angles", "the same interview", "multi angles", "alternate angle", "cut to the other angle", "angle switch", "change the angle", "two cameras" — switching to another angle means the picture changes but the **audio and lip-sync must stay matched** to the take.
