...
}), [], 'the default caption position sits inside the safe area');
...
assert.equal(merged.issues.filter((issue) => issue.code === 'caption_safe_area_horizontal').length, 1, 'appended issues are de-duplicated');
