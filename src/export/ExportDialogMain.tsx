...
function RenderBadge({ tab, renderEngine, engine, reason }: {
  ...
  const label = tab !== 'video' ? 'Local render'
    : engine ? engine.label
      : renderEngine === 'checking' ? 'Checking this device' : 'Adaptive local';
  const accelerated = tab !== 'video' || engine?.hardware;
  return <span className={`cc-export-local-badge${accelerated ? ' accelerated' : ''}`} title={reason ? reason : undefined}><i />{label}</span>;
}
...
function ExportMainHeader({ model }: { model: ExportDialogModel }) {
  ...
  return (
    <div className="cc-export-main-header">
      <div><h3>{activeTab.label}</h3><p>{activeTab.summary}</p></div>
...
function BackgroundExportJobs({ model }: { model: ExportDialogModel }) {
  ...
  return (
    <section className="cc-export-progress" aria-label="Background export tasks">
