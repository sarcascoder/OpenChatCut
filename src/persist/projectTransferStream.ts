...
export function base64ToBytes(value: string): Uint8Array {
  if (!value || value.length % 4 !== 0 || !/^[A-Za-z0-9+/]*={0,2}$/.test(value)) {
    throw new Error('Project package media base64 data is corrupt');
  }
  let binary: string;
  try { binary = atob(value); } catch { throw new Error('Project package media base64 data is corrupt'); }
...
async function* textLines(blob: Blob): AsyncGenerator<string> {
  ...
  try {
    for (;;) {
      ...
      if (pending.length > MAX_STREAM_LINE_CHARS && !pending.includes('\n')) throw new Error('Project package record exceeds the single-line limit');
      let newline = pending.indexOf('\n');
      while (newline >= 0) {
        if (newline > MAX_STREAM_LINE_CHARS) throw new Error('Project package record exceeds the single-line limit');
    ...
    if (pending.length > MAX_STREAM_LINE_CHARS) throw new Error('Project package record exceeds the single-line limit');
...
class StreamImportState {
  ...
  async consume(record: unknown, parseManifest: (value: unknown) => StreamProjectManifest): Promise<void> {
    ...
    if (row.type === 'media-start') {
      if (this.current) throw new Error('Project package media record was not closed');
      const entry = mediaManifestEntry(row);
      if (!entry) throw new Error('Project package media entry failed validation');
      if (this.packageSrcs.has(entry.src)) throw new Error(`Duplicate project package media src: ${entry.src}`);
      this.packageSrcs.add(entry.src);
      this.current = { entry, parts: [], bytes: 0 };
      return;
    }
    if (row.type === 'media-chunk') {
      if (!this.current || typeof row.data !== 'string') throw new Error('Project package media chunks are out of order');
      const bytes = base64ToBytes(row.data);
      this.current.bytes += bytes.byteLength;
      if (this.current.bytes > this.current.entry.bytes) throw new Error(`Project package media exceeds its declared size: ${this.current.entry.name}`);
    ...
    if (sameTarget && sameTarget.sha256 !== staged.sha256) throw new Error(`Project package media safe-name collision: ${staged.src}`);
  ...
  async finish(
    ...
    if (this.current) throw new Error('Project package media record is truncated');
...
export async function stageProjectStream(
  ...
  try {
    ...
    catch (cleanupError) { throw new AggregateError([error, cleanupError], 'Project package parsing failed, and cleaning up temporary media failed too'); }
    throw error;
  }
}
