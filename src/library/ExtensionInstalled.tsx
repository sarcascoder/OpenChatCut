  ...
  return (
    <div style={{ borderTop: `0.5px solid ${theme.border}`, padding: '9px 11px 11px' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(150px, 1fr))', gap: 4 }}>
        {pack.items.map((item) => (
          <div key={`${item.type}:${item.id}`} style={{ border: `0.5px solid ${theme.border}`, padding: '5px 7px', color: theme.text, fontSize: 10.5, borderRadius: 3 }}>
            <span style={{ color: theme.textDim }}>{EXTENSION_TYPE_LABEL[item.type] ?? item.type} · </span>
            {item.name}
          </div>
        ))}
      </div>
      <div style={{ display: 'flex', justifyContent: 'flex-end', gap: 6, marginTop: 9 }}>
        {confirming ? (
          <>
            <button type="button" disabled={busy} onClick={() => props.onRemove(pack)} style={{ ...secondaryButton(busy), color: theme.danger }}>Confirm uninstall</button>
            <button type="button" onClick={() => props.onConfirm(null)} style={secondaryButton()}>Cancel</button>
          </>
        ) : (
          <button type="button" onClick={() => props.onConfirm(pack.id)} style={{ ...secondaryButton(), color: theme.danger }}>Uninstall</button>
        )}
      </div>
    </div>
  );
    ...
    <article style={{ border: `0.5px solid ${theme.border}`, background: theme.panelAlt, borderRadius: 5, overflow: 'hidden', opacity: pack.enabled ? 1 : 0.72 }}>
      <div style={{ display: 'grid', gridTemplateColumns: 'auto minmax(0, 1fr) auto', alignItems: 'center', gap: '8px 10px', padding: 12 }}>
        ...
        <div style={{ minWidth: 0 }}>
          ...
          <div style={{ color: theme.textDim, fontSize: 10, marginTop: 3, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{pack.author || 'Unknown author'} · <SourceLabel pack={pack} /> · {new Date(pack.installedAt).toLocaleDateString()}</div>
        </div>
        ...
        <div style={{ gridColumn: '1 / -1', display: 'flex', alignItems: 'center', gap: 8, flexWrap: 'wrap' }}>
          <div style={{ display: 'flex', gap: 4, flexWrap: 'wrap', flex: '1 1 160px' }}>
            {packCounts(pack).map(([label, count]) => <ExtensionTag key={label}>{label} ×{count}</ExtensionTag>)}
          </div>
          <span style={{ color: theme.textDim, fontSize: 10, whiteSpace: 'nowrap' }}>{pack.enabled ? 'Enabled' : 'Disabled'}</span>
          <button type="button" onClick={() => props.onExpand(expanded ? null : pack.id)} aria-expanded={expanded} style={secondaryButton()}>
            {expanded ? 'Collapse' : 'View contents'}
          </button>
        </div>
      </div>
  ...
  if (!props.packs.length) {
    return (
      <div style={{ padding: 40, textAlign: 'center' }}>
        <div style={{ color: theme.text, fontSize: 12, fontWeight: 650 }}>No extensions installed yet</div>
        <div style={{ color: theme.textDim, fontSize: 10.5, marginTop: 5 }}>Install extensions from Discover. Their content will automatically appear in the matching resource categories.</div>
      </div>
    );
  }
