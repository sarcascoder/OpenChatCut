...
assert.match(source, /aria-label=\{`Add to timeline: \$\{template\.name\}`\}/);
assert.match(source, /<Icon name="plus"/);
assert.doesNotMatch(source, /className="cc-template-add"\s+onClick=/);
assert.match(source, /title="More actions"/, 'the visible menu button should remain keyboard accessible');
