...
// ── 2. word strike → deletion (deleting a word deletes the video) ──
{
  const d = makeDraft(doc(state));
...
// ── 4. restore: re-adding previously deleted words ──
{
  ...
  assert.ok(r.changes.some((c) => c.includes('restored')), 'restore audited');
}
