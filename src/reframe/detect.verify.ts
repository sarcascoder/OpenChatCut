...
// Only tests DOM-free pure math (no real video/network): focalFromEnergyGrid / magnificationForAspect /
// energyGridFromImageData / sampleFrames, and checks that the emitted keyframe fields land in the ReframeKeyframe value ranges.
...
// —— focalFromEnergyGrid: a synthetic high-energy blob → the centroid lands at that cell's center ——
...
// focal point always within 0..1
assert.ok(f.x >= 0 && f.x <= 1 && f.y >= 0 && f.y <= 1, 'focal in 0..1');

// —— magnificationForAspect: 16:9→9:16 needs ~3.16 magnification to fill; same ratio = 1; clamp ——
...
// —— energyGridFromImageData: one high-contrast checkerboard cell → that cell has the highest energy, the centroid moves toward it ——
const W = 8;
const H = 4;
...
for (let y = 0; y < H; y++) {
  for (let x = 0; x < W; x++) {
    const i = (y * W + x) * 4;
    // top-right area (x>=6, y<2) gets a strong checkerboard → high variance; the rest pure black → zero variance
...
// —— sampleFrames: strictly increasing, includes first and last, bounded by maxSamples ——
...
// —— the emitted keyframe record matches the ReframeKeyframe fields/value ranges ——
const record: DetectedKeyframe = { frame: fs[1], focalPointX: f.x, focalPointY: f.y, magnification: mag916 };
const asKeyframe: ReframeKeyframe = record; // structure must match exactly (checked at compile time)
...
// —— smoothFocalPath: EMA stays in range, a higher smooth sticks closer to the previous frame ——
