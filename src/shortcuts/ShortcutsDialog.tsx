...
export function ShortcutsDialog({ onClose }: ShortcutsDialogProps) {
  const actionLabel = (a: Pick<ShortcutAction, 'label'>): string => a.label;
    ...
    >
      ...
      >
        <div style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '14px 16px', borderBottom: `0.5px solid ${theme.border}` }}>
          ...
          <b style={{ fontSize: 14, flex: 1 }}>Keyboard Shortcuts</b>
          <span style={{ fontSize: 11, color: theme.textDim }}>Click a shortcut to rebind</span>
          {customizedCount() > 0 && (
          ...
          <button type="button" onClick={onClose} title="Close" style={{ background: 'none', border: 'none', cursor: 'pointer', color: theme.textDim, padding: 4, display: 'grid' }}>
            <Icon name="x" size={16} />
          </button>
        </div>
          ...
          {SHORTCUT_GROUPS.map((g) => {
            ...
            return (
              <div key={g.id} style={{ marginTop: 12 }}>
                <div style={{ fontSize: 11, color: theme.textDim, letterSpacing: 0.4, margin: '0 4px 6px' }}>{g.label}</div>
            ...
            <div key={a.id} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '7px 10px', borderRadius: 3, background: theme.panelAlt }}>
                        ...
                        {isCustomized(a.id) && !capturing && (
                          <button type="button" title="Restore default" onClick={() => resetBinding(a.id)}
                        ...
                        >
                          {capturing ? 'Press keys… (Esc)' : showKeys(a.keys)}
                        </button>
                        {conflicting && pending && (
      <div style={{ position: 'absolute', right: 24, marginTop: 40, zIndex: 1, background: theme.panel, border: `0.5px solid ${theme.accent}`, borderRadius: 4, padding: 10, boxShadow: `0 8px 24px ${themeAlpha.shadow(0.67)}`, maxWidth: 300 }}>
                            <div style={{ fontSize: 11.5, marginBottom: 6 }}>
                              <b>{showKeys(pending.keys)}</b> {'is already used by: '}{pending.conflicts.map(actionLabel).join(', ')}
                            </div>
                            <div style={{ display: 'flex', gap: 6, justifyContent: 'flex-end' }}>
                              <button type="button" onClick={() => setPending(null)} style={{ fontSize: 11, background: 'none', border: `0.5px solid ${theme.border}`, borderRadius: 6, padding: '3px 8px', color: theme.text, cursor: 'pointer' }}>Cancel</button>
                              <button type="button" onClick={() => { setBinding(pending.id, pending.keys); setPending(null); }} style={{ fontSize: 11, background: theme.accent, border: 'none', borderRadius: 6, padding: '3px 8px', color: theme.onAccent, cursor: 'pointer' }}>Bind anyway</button>
                            </div>
                          </div>
                        )}
                      </div>
